...
The equivalence holds **exactly for the coefficient** — this is the theorem, and we verify it to five decimal places. The **standard error needs one correction**: the third-stage regression `Ỹ ~ T̃` computes its SE using *n − 2* residual degrees of freedom, but the controls consumed *k − 2* more in the residualization steps. Dividing by √(n − k) instead of √(n − 2) recovers the full-model SE exactly. Notebook 02 shows the naive third-stage SE is therefore *not* identical (it is slightly too small), and then reproduces the correct SE via the closed-form formula below. Verifying all of this numerically on real data demonstrates that regression removes confounding by physically orthogonalizing the treatment with respect to the controls, not by "holding them constant" in any abstract sense.
...
1. **CIA is not fully plausible.** Unobserved ability components, family networks, and school quality remain uncontrolled. The estimated ~5.4% return per year should be read with caution: remaining ability bias pushes it *up*, while classical measurement error in self-reported schooling pushes it *down*. The net direction is theoretically ambiguous — and the fact that Card (1995)'s IV estimate is *higher* than this OLS suggests attenuation is non-trivial here, so this should not be treated as a clean upper bound.

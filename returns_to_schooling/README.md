...
**The question:** What is the causal return to an additional year of schooling? Naive OLS says ~6%. With experience, demographics, and IQ controlled, it falls to ~4.5%. The 1.5 percentage-point gap is ability bias — and we calculate it exactly from three separate regressions using the Omitted Variable Bias formula, verifying it matches to ten decimal places.
...
| Coefficient and standard error identity across all three FWL steps | 02 |
...
|---------|-------|---------------|
| [`01_naive_ols_and_confounding`](notebooks/01_naive_ols_and_confounding.ipynb) | OVB formula | Including IQ drops the return to schooling from 6% → ~4.5%. The OVB formula recovers the missing gap exactly. |
| [`02_fwl_orthogonalization`](notebooks/02_fwl_orthogonalization.ipynb) | FWL theorem | All three approaches give identical coefficients to five decimal places. Only full orthogonalization gives the correct SE. |
...
The OVB numerical verification (Notebook 01). Naive OLS overstates the return to schooling by exactly 1.47 percentage points. We know it's exactly that because we calculated δ × γ from three regressions and it matches to ten decimal places. Raises the IV puzzle at the end: if OLS is biased upward, why is Card's IV estimate higher?
